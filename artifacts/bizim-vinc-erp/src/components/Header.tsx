            ...
            return (
              ...
              >
                ...
                {item.badge !== undefined && (
                  <span className="ml-0.5 px-1.5 py-0.2 text-[10px] font-bold rounded-full bg-emerald-600 text-white">{item.badge}</span>
                )}
              </button>
            );
          })}
          ...
          {visibleNavItems.slice(0, 5).map((item) => {
            ...
            return (
              ...
              >
                ...
                <span>{item.label}</span>
                {item.badge !== undefined && (
                  <span className="px-1.5 py-0.2 text-[9px] font-bold rounded-full bg-emerald-600 text-white">{item.badge}</span>
                )}
              </button>
            );
          })}
        ...
        <div className="flex items-center gap-2">
          <div className="relative">
            ...
            >
              <Bell size={16} />
              {unreadCount > 0 && (
                <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-emerald-600 text-white text-[10px] font-bold flex items-center justify-center shadow-xs">{unreadCount}</span>
              )}
            </button>
            {showNotifications && (
              <div className="absolute right-0 mt-2 w-80 bg-white border border-emerald-100 rounded-2xl shadow-2xl p-3 z-50 animate-in fade-in duration-150">
                <div className="flex items-center justify-between pb-2 border-b border-emerald-100 mb-2">
                  ...
                  <button onClick={() => setShowNotifications(false)} className="text-slate-400 hover:text-slate-600 text-xs font-medium">Kapat</button>
                </div>
                    ...
                    notifications.map((n) => (
                      <div
                        ...
                        className={`p-2.5 rounded-xl border text-xs cursor-pointer transition ${
                          n.isRead ? 'bg-slate-50 border-slate-200/70 text-slate-500' : 'bg-emerald-50/80 border-emerald-200 text-emerald-950 font-medium shadow-2xs'
                        }`}
                      >
                        <div className="flex items-center justify-between mb-1">
                          <span className="font-bold text-slate-800">{n.title}</span>
                          <span className="text-[10px] text-slate-400 font-mono">{new Date(n.createdAt).toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' })}</span>
                        </div>
          ...
          >
            ...
            <span className={`w-1.5 h-1.5 rounded-full ${isSupabaseOnline ? 'bg-emerald-500 animate-pulse' : 'bg-emerald-500'}`} />
          </button>
            ...
            return (
              <button
                ...
                className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-xs font-semibold transition ${
                  isActive ? 'bg-emerald-600 text-white font-bold' : 'text-slate-700 hover:bg-emerald-50'
                }`}
              >
                ...
                {item.badge !== undefined && (
                  <span className={`px-2 py-0.5 text-[10px] font-bold rounded-full ${isActive ? 'bg-white text-emerald-800' : 'bg-emerald-100 text-emerald-800'}`}>{item.badge}</span>
                )}
              </button>
            );
          })}
