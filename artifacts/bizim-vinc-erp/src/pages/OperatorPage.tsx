    ...
    <div className="max-w-4xl mx-auto space-y-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-xl font-bold text-emerald-950">{myPerson.fullName}</h1>
                <span className="px-2 py-0.5 rounded-full text-[11px] font-mono font-bold bg-emerald-500/10 text-emerald-600 border border-emerald-500/20 uppercase">
                  {myPerson.kind}
                </span>
              </div>
              <p className="text-xs text-slate-600 mt-0.5">
                {myPerson.title} • Sicil No: <span className="font-mono text-emerald-950">{myPerson.employeeNo}</span> • Tel: {myPerson.phone}
              </p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3">
        <button
          ...
          className="p-4 rounded-2xl bg-white border border-emerald-100 hover:border-emerald-500/40 transition flex flex-col items-center justify-center text-center gap-2 group hover:bg-emerald-500/5 shadow-sm"
        >
          ...
          <div>
            <div className="text-xs font-bold text-emerald-950">Yoklama Ver</div>
            <div className="text-[11px] text-slate-600">Şantiyeye Vardım</div>
          </div>
        </button>
        ...
        >
          ...
          <div>
            <div className="text-xs font-bold text-emerald-950">Mesai Bildir</div>
            <div className="text-[11px] text-slate-600">Fazla Çalışma Talebi</div>
          </div>
        </button>

        <button
          onClick={() => setActiveModal('avans')}
          className="p-4 rounded-2xl bg-white border border-emerald-100 hover:border-sky-500/40 transition flex flex-col items-center justify-center text-center gap-2 group hover:bg-sky-500/5 shadow-sm"
        >
          ...
          <div>
            <div className="text-xs font-bold text-emerald-950">Avans İste</div>
            <div className="text-[11px] text-slate-600">Yol / Saha Harcırahı</div>
          </div>
        </button>

        <button
          onClick={() => setActiveModal('izin')}
          className="p-4 rounded-2xl bg-white border border-emerald-100 hover:border-purple-500/40 transition flex flex-col items-center justify-center text-center gap-2 group hover:bg-purple-500/5 shadow-sm"
        >
          ...
          <div>
            <div className="text-xs font-bold text-emerald-950">İzin Talebi</div>
            <div className="text-[11px] text-slate-600">Yıllık / Mazeret İzni</div>
          </div>
        </button>
        <button
        ...
        >
          ...
          <div><div className="text-xs font-bold text-emerald-950">Makbuz / Fiş Kes</div><div className="text-[11px] text-slate-600">İşi onaya gönder</div></div>
        </button>
        <button
          onClick={() => setActiveModal('masraf')}
          className="p-4 rounded-2xl bg-white border border-emerald-100 hover:border-rose-500/40 transition flex flex-col items-center justify-center text-center gap-2 group hover:bg-rose-500/5 shadow-sm"
        >
          ...
          <div><div className="text-xs font-bold text-emerald-950">Masraf Gir</div><div className="text-[11px] text-slate-600">Saha gideri bildir</div></div>
        </button>
      </div>
      ...
      <div className="bg-white border border-emerald-100 rounded-2xl p-5 shadow-sm space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-bold text-emerald-950">Son Bildirimlerim ve Onay Durumları</h2>
          </div>
          <span className="text-xs text-slate-600">{myApprovals.length} Talep</span>
        </div>
              ...
              return (
                <div key={app.id} className="py-3 flex items-start justify-between gap-3">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      <span className="font-semibold text-xs text-emerald-950">{app.title}</span>
                      <span className="text-[10px] px-2 py-0.5 rounded-full uppercase font-mono bg-emerald-50 text-slate-600">
                        {app.kind}
                      </span>
                    </div>
                    {app.note && <div className="text-xs text-slate-600">{app.note}</div>}
                    {app.rejectionReason && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white border border-emerald-100 text-emerald-950 rounded-2xl w-full max-w-md p-6 shadow-2xl space-y-4">
            ...
            <form onSubmit={handleSendAdvance} className="space-y-4">
              <div>
                <label className="block text-xs text-emerald-900 mb-1">Talep Edilen Tutar (₺) *</label>
                <input
                  type="number"
                  required
                  value={advanceAmount}
                  onChange={(e) => setAdvanceAmount(Number(e.target.value))}
                  className="w-full px-3 py-2 rounded-xl bg-emerald-50 border border-emerald-100 text-sm font-bold text-emerald-600 focus:outline-none focus:border-emerald-500"
                />
              </div>

              <div>
                <label className="block text-xs text-emerald-900 mb-1">Talep Gerekçesi / Açıklama</label>
                <textarea
                  value={advanceNote}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white border border-emerald-100 text-emerald-950 rounded-2xl w-full max-w-md p-6 shadow-2xl space-y-4">
            ...
            <form onSubmit={handleSendLeave} className="space-y-3.5">
              <div>
                <label className="block text-xs text-emerald-900 mb-1">İzin Türü *</label>
                <select
                  value={leaveType}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs text-emerald-900 mb-1">Başlangıç Tarihi *</label>
                  <input
                    type="date"
                    required
                    value={leaveStart}
                ...
                <div>
                  <label className="block text-xs text-emerald-900 mb-1">Bitiş Tarihi *</label>
                  <input
                    type="date"
                    required
                    value={leaveEnd}
              ...
              <div>
                <label className="block text-xs text-emerald-900 mb-1">Gün Sayısı</label>
                <input
                  type="number"
                  min={1}
                  value={leaveDays}
              ...
              <div>
                <label className="block text-xs text-emerald-900 mb-1">Açıklama</label>
                <textarea
                  value={leaveDesc}
      ...
      {activeModal === 'makbuz' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white border border-emerald-100 text-emerald-950 rounded-2xl w-full max-w-md p-6 shadow-2xl space-y-4">
            ...
            <p className="text-xs text-slate-600">Makbuz, cari kart ve vinç bilgisiyle yönetici onayına gönderilir.</p>
            <form onSubmit={handleSendJobReceipt} className="space-y-3">
              <input type="number" min="1" required value={jobAmount || ''} onChange={(e) => setJobAmount(Number(e.target.value))} placeholder="Tutar (₺)" className="w-full px-3 py-2 rounded-xl bg-emerald-50 border border-emerald-100 text-sm" />
              <textarea value={jobDescription} onChange={(e) => setJobDescription(e.target.value)} placeholder="Yapılan iş / şantiye notu" rows={3} className="w-full px-3 py-2 rounded-xl bg-emerald-50 border border-emerald-100 text-xs" />
              <div className="flex justify-end gap-2"><button type="button" onClick={() => setActiveModal('none')} className="px-4 py-2 rounded-xl text-xs text-slate-600 bg-emerald-50">Vazgeç</button><button type="submit" className="px-4 py-2 rounded-xl text-xs font-bold bg-emerald-500 text-neutral-950">Onaya Gönder</button></div>
            </form>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white border border-emerald-100 text-emerald-950 rounded-2xl w-full max-w-md p-6 shadow-2xl space-y-4">
            ...
            <form onSubmit={handleSendExpense} className="space-y-3">
              <input type="number" min="1" required value={expenseAmount || ''} onChange={(e) => setExpenseAmount(Number(e.target.value))} placeholder="Tutar (₺)" className="w-full px-3 py-2 rounded-xl bg-emerald-50 border border-emerald-100 text-sm" />
              <textarea value={expenseDetail} onChange={(e) => setExpenseDetail(e.target.value)} placeholder="Masraf açıklaması" rows={3} className="w-full px-3 py-2 rounded-xl bg-emerald-50 border border-emerald-100 text-xs" />
              <div className="flex justify-end gap-2"><button type="button" onClick={() => setActiveModal('none')} className="px-4 py-2 rounded-xl text-xs text-slate-600 bg-emerald-50">Vazgeç</button><button type="submit" className="px-4 py-2 rounded-xl text-xs font-bold bg-rose-500 text-neutral-950">Masrafı Kaydet</button></div>
            </form>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white border border-emerald-100 text-emerald-950 rounded-2xl w-full max-w-md p-6 shadow-2xl space-y-4">
            ...
            <form onSubmit={handleSendOvertime} className="space-y-3.5">
              <div>
                <label className="block text-xs text-emerald-900 mb-1">Mesai Tarihi *</label>
                <input
                  type="date"
                  required
                  value={overtimeDate}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs text-emerald-900 mb-1">Başlangıç Saati</label>
                  <input
                    type="time"
                    value={overtimeStart}
                ...
                <div>
                  <label className="block text-xs text-emerald-900 mb-1">Bitiş Saati</label>
                  <input
                    type="time"
                    value={overtimeEnd}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs text-emerald-900 mb-1">Toplam Saat</label>
                  <input
                    type="number"
                    step="0.5"
                    value={overtimeHours}
                ...
                <div>
                  <label className="block text-xs text-emerald-900 mb-1">Mesai Türü</label>
                  <select
                    value={overtimeType}
              ...
              <div>
                <label className="block text-xs text-emerald-900 mb-1">Yapılan İş / Şantiye Notu</label>
                <textarea
                  value={overtimeDesc}
