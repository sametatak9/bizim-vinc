...
export const ApprovalModal: React.FC<ApprovalModalProps> = ({ approval, isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-150">
      <div className="bg-white border border-emerald-100 text-emerald-950 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
        <div className="px-6 py-4 bg-emerald-50 border-b border-emerald-100 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 font-bold flex items-center justify-center text-sm">
              {approval.personInitials || 'OP'}
            </div>
            <div>
              <h2 className="text-base font-bold text-emerald-950">Talep İnceleme & Onay</h2>
              <p className="text-xs text-slate-600">{approval.personName}</p>
            </div>
          </div>
          <button
            onClick={onClose}
            className="p-1.5 rounded-lg text-slate-600 hover:text-white hover:bg-emerald-50 transition"
          >
            <X size={18} />
          </button>
        </div>
        ...
        <div className="p-6 space-y-4">
          <div className="bg-emerald-50 rounded-xl p-4 border border-emerald-100 space-y-2.5">
            ...
            <div className="text-sm font-bold text-emerald-950">{approval.title}</div>
            ...
            {approval.note && (
              <div className="text-xs text-emerald-900 pt-1.5 border-t border-emerald-100">
                "{approval.note}"
              </div>
            )}
          ...
          {!isRejecting ? (
            <div className="space-y-3">
              <div>
                <label className="block text-xs text-emerald-900 mb-1">
                ...
                <textarea
                  ...
                  rows={2}
                  className="w-full px-3 py-2 bg-emerald-50 border border-emerald-100 rounded-xl text-xs text-emerald-950 focus:outline-none focus:border-emerald-500"
                />
              </div>
            ...
            <div className="space-y-3">
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full px-3 py-2 bg-emerald-50 border border-rose-500/40 rounded-xl text-xs text-emerald-950 focus:outline-none focus:border-rose-500"
                />
              </div>
              ...
              <div className="flex items-center gap-2 pt-1">
                <button
                  ...
                  className="px-4 py-2.5 bg-emerald-50 text-slate-600 hover:text-emerald-950 rounded-xl text-xs transition"
                >
                  Geri
                </button>
                <button
